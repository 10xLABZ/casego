...
async function loadAdminSettings(){if(!window.casegoIsFirmAdmin)return;document.querySelectorAll('.admin-only-settings').forEach(x=>x.hidden=false);const roles=await roleCatalog();await Promise.all([loadRolesPanel(roles),loadUsers(roles),loadBillingDefaults()]);setupCreateUser(roles);}
