...
async function tasks(){return window.CaseGOWorkspace.tasksPage();}
...
async function setupCreateUser(roles){
 const modal=$('addUserModal'),button=$('createUserButton'),message=$('createUserStatus'),roleSelect=$('createUserRole');
 roleSelect.innerHTML='<option value="">Choose role</option>'+roles.map(r=>`<option value="${r.id}">${e(r.name)}</option>`).join('');
 let chooser=$('createUserPermissions');if(!chooser){chooser=document.createElement('section');chooser.id='createUserPermissions';chooser.className='create-user-permissions';chooser.innerHTML='<div class="permission-chooser-head"><strong>ACCOUNT FEATURES</strong><span><button type="button" data-all>Select all</button><button type="button" data-none>Clear optional</button></span></div><div class="permission-check-grid" data-permission-list></div>';modal.querySelector('.secure-invite-note').before(chooser);}
 const {data:permissions,error:permissionError}=await sb().from('permissions').select('id,permission_key,name,description').order('name');if(permissionError){message.textContent='Permission features could not load.';return;}
 const {data:rolePermissions}=await sb().from('role_permissions').select('role_id,permission_id,allowed').in('role_id',roles.map(r=>r.id));
 const list=chooser.querySelector('[data-permission-list]');list.innerHTML=(permissions||[]).map(p=>`<label class="permission-choice"><input type="checkbox" value="${e(p.permission_key)}" data-permission-id="${p.id}" ${p.permission_key==='expenses.create'?'checked disabled':''}><span><b>${e(p.name)}</b><small>${e(p.description||'')}</small></span></label>`).join('');
 const applyRole=()=>{const selected=new Set((rolePermissions||[]).filter(x=>x.role_id===roleSelect.value&&x.allowed).map(x=>x.permission_id));list.querySelectorAll('input').forEach(input=>input.checked=input.value==='expenses.create'||selected.has(input.dataset.permissionId));};
 roleSelect.onchange=applyRole;chooser.querySelector('[data-all]').onclick=()=>list.querySelectorAll('input').forEach(x=>x.checked=true);chooser.querySelector('[data-none]').onclick=()=>list.querySelectorAll('input').forEach(x=>x.checked=x.value==='expenses.create');
 $('addUserButton').onclick=()=>{message.textContent='';modal.hidden=false;applyRole();};const close=()=>{if(!button.disabled)modal.hidden=true;};$('closeAddUserModal').onclick=close;$('cancelCreateUserButton').onclick=close;modal.onclick=ev=>{if(ev.target===modal)close();};
 button.onclick=async()=>{const payload={firm_id:firmId(),first_name:$('createUserFirstName').value.trim(),last_name:$('createUserLastName').value.trim(),email:$('createUserEmail').value.trim(),password:$('createUserPassword').value,role_id:roleSelect.value,permission_keys:[...list.querySelectorAll('input:checked')].map(x=>x.value)};if(!payload.email||payload.password.length<8||!payload.role_id){message.textContent='Enter an email, temporary password of at least 8 characters, and role.';return;}button.disabled=true;message.textContent='Creating user…';try{const {data,error}=await sb().functions.invoke('casego-create-user',{body:payload});if(error)throw new Error(data?.error||error.message);if(data?.error)throw new Error(data.error);message.textContent='User created with the selected features.';for(const id of ['createUserFirstName','createUserLastName','createUserEmail','createUserPassword'])$(id).value='';roleSelect.value='';await loadUsers(roles);setTimeout(()=>{button.disabled=false;modal.hidden=true;},650);}catch(err){message.textContent=err.message||'Unable to create user.';button.disabled=false;}};
}
